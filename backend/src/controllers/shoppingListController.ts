...
const WEIGHT_UNITS = ["mg", "g", "kg", "oz", "lb", "pinch", "handful", "serving"];
...
const METRIC_CONVERSIONS = {
  volume: {
    ...
    rules: [
      ...
      { unit: "tsp", factor: 5 },
      { unit: "tbsp", factor: 15 },
      { unit: "fl oz", factor: 28.4131 },
      { unit: "cup", factor: 250 },
      { unit: "pint", factor: 568.261 },
      { unit: "quart", factor: 1136.52 },
      { unit: "gallon", factor: 4546.09 },
    ],
  },
  weight: {
    base: "g",
    ...
    for (const row of entryResult.rows) {
      ...
      for (const ing of ingredients) {
        ...
        if (!metric || metric.amount == null) {
          // check if created recipe then, diff format
          if ("amount" in ing && "unit" in ing) {
            qty = ing.amount;
          ...
          qty = metric.amount;
          unit = (metric.unitShort || "").toLowerCase();
        }
